...
import { Camera, Save, Lock, User, Phone, MapPin, Hash, Loader2, Globe, CheckCircle2, Clock, ExternalLink } from 'lucide-react';
  ...
  <div className="space-y-1.5">
    ...
    <input
      ...
      style={{
        ...
        background: disabled ? `${NAV}05` : 'white',
        color: NAV,
        outline: 'none',
...
const CoordinatorSettingsPage = () => {
  ...
  useEffect(() => {
    if (!user?.id) return;
    supabase.from('site_requests').select('id, status, website_url, requested_at')
      .eq('coordinator_id', user.id).maybeSingle()
  ...
  const handleRequestSite = async () => {
    ...
    try {
      const { data, error } = await supabase.from('site_requests')
        .insert({ coordinator_id: user.id, status: 'PENDENTE' })
        .select('id, status, website_url, requested_at').single();
      if (error) throw error;
      setSiteRequest(data);
  ...
  return (
    <>
      <Helmet><title>Dados do Coordenador — Novos Autores do Brasil</title></Helmet>
      ...
      <div className="space-y-6 pb-12 max-w-2xl mx-auto">
        ...
        <BrandCard>
          ...
          <div className="px-6 py-6 space-y-4">
            ...
            <div className="pt-2">
              <BtnPrimary
                onClick={handleSaveProfile} disabled={savingProfile} loading={savingProfile}
        ...
        {/* Site de Divulgação */}
        <BrandCard>
          <BrandCardHeader icon={Globe} iconColor={BLUE} accentColor={BLUE} title="Site de Divulgação" />
          <div className="px-6 py-6">
            {(!siteRequest || siteRequest.status === 'CANCELADO') && (
              <div className="flex flex-col gap-3">
                ...
                <BtnPrimary
                  onClick={handleRequestSite} disabled={requestingSite} loading={requestingSite}
                  icon={Globe} label="Solicitar Agora" loadingLabel="Enviando..."
                />
              </div>
            )}
            {siteRequest?.status === 'PENDENTE' && (
              <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: 'rgba(245,158,11,0.07)', border: '1px solid rgba(245,158,11,0.25)' }}>
                <Clock className="h-5 w-5 shrink-0 mt-0.5" style={{ color: '#F59E0B' }} />
                <div>
                  <p className="font-semibold text-sm" style={{ color: '#92640A' }}>Solicitação em análise</p>
                  <p className="text-xs mt-0.5" style={{ color: '#B45309' }}>Nossa equipe recebeu seu pedido e entrará em contato em breve.</p>
                </div>
              </div>
            )}
            {siteRequest?.status === 'EM_ANDAMENTO' && (
              <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: `${BLUE}08`, border: `1px solid ${BLUE}25` }}>
                <Loader2 className="h-5 w-5 shrink-0 mt-0.5 animate-spin" style={{ color: BLUE }} />
                <div>
                  <p className="font-semibold text-sm" style={{ color: BLUE }}>Site em desenvolvimento</p>
                  <p className="text-xs mt-0.5" style={{ color: `${BLUE}90` }}>Você será notificado quando estiver pronto.</p>
                </div>
              </div>
            )}
            {siteRequest?.status === 'CONCLUIDO' && (
              <div className="flex items-start gap-3 p-4 rounded-xl" style={{ background: 'rgba(16,185,129,0.07)', border: '1px solid rgba(16,185,129,0.25)' }}>
                <CheckCircle2 className="h-5 w-5 shrink-0 mt-0.5" style={{ color: '#10B981' }} />
                <div className="flex-1">
                  <p className="font-semibold text-sm" style={{ color: '#065F46' }}>Site pronto!</p>
                  {siteRequest.website_url ? (
                    <a href={siteRequest.website_url} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 text-sm hover:underline font-medium mt-1"
                      style={{ color: '#10B981' }}>
                      <ExternalLink className="h-3.5 w-3.5" />
                      {siteRequest.website_url}
                    </a>
                  ) : (
                    <p className="text-xs mt-0.5" style={{ color: '#10B981' }}>Seu site está no ar!</p>
                  )}
                </div>
              </div>
            )}
        ...
        <BrandCard>
          ...
          <div className="px-6 py-6 space-y-4">
            ...
            <div className="pt-2">
              <BtnOutline
                onClick={handleChangePassword} disabled={savingPassword} loading={savingPassword}
